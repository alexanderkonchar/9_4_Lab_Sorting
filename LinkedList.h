//
// Created by Alex on 3/27/23.
//

#ifndef INC_9_4_LAB_SORTING_LINKEDLIST_H
#define INC_9_4_LAB_SORTING_LINKEDLIST_H
...
void LinkedList<Type>::mergeSort(shared_ptr<Node<Type> > &head) {
    if (!(head) || !((head)->next)) return; // Base case - List has 0 or 1 elements
    auto middle = getMiddle(head); // Find the middle node of the linked list
    auto left = head;
    ...
    head = mergeInOrder(left, right); // Merge the two sorted halves
}
...
shared_ptr<Node<Type> > LinkedList<Type>::mergeInOrder(shared_ptr<Node<Type> > a, shared_ptr<Node<Type> > b) {
    ...
    if (a->data < b->data) {
        head = a;
        head->next = mergeInOrder(a->next, b); // Recursively merge the rest of the list
    }
    else {
        head = b;
        head->next = mergeInOrder(a, b->next); // Recursively merge the rest of the list
    }
    return head;
}

#endif //INC_9_4_LAB_SORTING_LINKEDLIST_H
